...
if IN_COLAB:
    ...
    if "/content/pyTrackers" not in sys.path:
        ...
        print(sys.path)
    if "/content/pyTrackers/MixFormer" not in sys.path:
        print("**** path not set ****")
        sys.path.insert(0, "/content/pyTrackers/MixFormer")
        print(sys.path)
...
if __name__ == '__main__':
    ...
    for data_name in data_names:
        ...
        if tracker_tomp.viot:
            write_results(data_name, 'tomp', tomp_results, viot="_viot")
        else:
        ...
        if tracker_kcf_hog.viot:
            write_results(data_name, 'kcf_hog', kcf_hog_results, viot="_viot")
        else:
            write_results(data_name, 'kcf_hog', kcf_hog_results)
        print('kcf hog done!')

        if tracker_ldes.viot:
            ldes_preds=tracker_ldes.tracking(verbose=True,video_path="../results/ldes_{:s}_viot.mp4".format(data_name))
        else:
        ...
        if tracker_ldes.viot:
            write_results(data_name, 'ldes', ldes_results, viot="_viot")
        else:
            write_results(data_name, 'ldes', ldes_results)
        print('ldes done!')

        if tracker_csrdcf.viot:
            csrdcf_preds=tracker_csrdcf.tracking(verbose=True,video_path="../results/csrdcf_{:s}_viot.mp4".format(data_name))
        else:
        ...
        if tracker_csrdcf.viot:
            write_results(data_name, 'csrdcf', csrdcf_results, viot="_viot")
        else:
            write_results(data_name, 'csrdcf', csrdcf_results)
        print('csrdcf done!')

        if tracker_strcf.viot:
            strcf_preds=tracker_strcf.tracking(verbose=True,video_path="../results/strcf_{:s}_viot.mp4".format(data_name))
        else:
        ...
        if tracker_strcf.viot:
            write_results(data_name, 'strcf', strcf_results, viot="_viot")
        else:
